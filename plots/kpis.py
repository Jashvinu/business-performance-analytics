...
def indicator_chart(label, current_value, delta_value, y_data, trace_name,
    ...
    fig.add_trace(
        go.Scatter(x=x_data, y=y_data, mode="lines", fill='tozeroy', name=trace_name)
    )
    ...
    fig.update_layout(height=250, hovermode="x unified",
                      hoverlabel=dict(bgcolor="white", font_color="black", font_size=16, font_family="Rockwell")
                      )
...
def get_num_of_customers(current_data: pd.DataFrame, previous_data: pd.DataFrame) -> go.Figure:
    ...
    fig = indicator_chart(label="Number of Customers", current_value=current_value, delta_value=previous_value,
                          x_data=y_data["Month"], y_data=y_data["Customer ID"], trace_name="# Customers")
    return fig
...
def get_clv(current_data: pd.DataFrame, previous_data: pd.DataFrame) -> go.Figure:
    ...
    :param current_data: DataFrame containing the current customer data.
                         with columns "Customer ID" and "Valuation Date" & "CLTV Monetary Value".
    :param previous_data: DataFrame containing the previous customer data.
                          with columns "Customer ID" and "Valuation Date" & "CLTV Monetary Value".
    :return: A Plotly Figure object containing the indicator chart.
    """
    # Calculating the total monetary value generated by each customer.
    current = current_data.groupby(['Customer ID'])['CLTV Monetary Value'].sum()
    # Averaging it across the data
    current = current.mean()
    # monetary value of each customer in previous period/data (for reference purpose)
    previous = previous_data.groupby(['Customer ID'])['CLTV Monetary Value'].sum()
    ...
    y_data = current_data.groupby("Valuation Date")["CLTV Monetary Value"].sum().reset_index()
    # Adding Month
    y_data['Month'] = y_data['Valuation Date'].dt.month
    # Getting monthly trend
    y_data = y_data.groupby("Month")["CLTV Monetary Value"].mean().reset_index()
    y_data['Month'] = y_data['Month'].apply(lambda x: MONTHS[x - 1])
...
def average_life_span(current_data: pd.DataFrame, previous_data: pd.DataFrame) -> go.Figure:
    ...
    cust_lsp_current = current_data.groupby('Customer ID')['Valuation Date'].agg(['min', 'max']).reset_index()
    # lifespan of each customer as the difference between the first and last active date
    cust_lsp_current['lifespan'] = cust_lsp_current['max'] - cust_lsp_current['min']
    # lifespan as the number of days
    cust_lsp_current['lifespan'] = cust_lsp_current['lifespan'].apply(lambda x: x.days)
    ...
    cust_lsp_prev = previous_data.groupby('Customer ID')['Valuation Date'].agg(['min', 'max']).reset_index()
    cust_lsp_prev['lifespan'] = cust_lsp_prev['max'] - cust_lsp_prev['min']
    cust_lsp_prev['lifespan'] = cust_lsp_prev['lifespan'].apply(lambda x: x.days)
    # average lifespan of the customers for previous period
    avg_lsp_prev = cust_lsp_prev['lifespan'].mean()
    # Merging the customer lifespan data with the current period data on 'Customer ID'
    cust_lsp_current = cust_lsp_current.merge(current_data, on="Customer ID")
    # Aggregating by 'Valuation Date' and calculating the average lifespan for each date
    y_data = cust_lsp_current.groupby("Valuation Date")["lifespan"].mean().reset_index()
...
def average_arpu(current_data: pd.DataFrame, previous_data: pd.DataFrame) -> go.Figure:
    ...
    :param current_data: DataFrame containing the current customer data.
                         with columns "Customer ID" and "Valuation Date" & "Total Revenue".
    :param previous_data: DataFrame containing the previous customer data.
                          with columns "Customer ID" and "Valuation Date" & "Total Revenue".
    :return: A Plotly Figure object containing the indicator chart.
    """
    # Calculating the current period's Average Revenue Per User (ARPU)
    current_arpu = current_data["Total Revenue"].sum() / current_data['Customer ID'].nunique()
    # Calculating the previous period's Average Revenue Per User (ARPU)
    previous_arpu = previous_data["Total Revenue"].sum() / previous_data['Customer ID'].nunique()
    # Grouping by 'Valuation Date' for 'Total Revenue' and the number of unique 'Customer ID'
    arpu_data = current_data.groupby("Valuation Date").agg(
        {'Total Revenue': 'sum', 'Customer ID': 'nunique'}
    ).reset_index()
    # ARPU for each 'Valuation Date'
    arpu_data["arpu"] = arpu_data["Total Revenue"] / arpu_data["Customer ID"]
...
def churn_rate(current_data: pd.DataFrame, previous_data: pd.DataFrame) -> go.Figure:
    ...
    :param current_data: DataFrame containing the current customer data.
                         with columns "Customer ID", "Valuation Date", and "P notAlive".
    :param previous_data: DataFrame containing the previous customer data.
                          with columns "Customer ID", "Valuation Date", and "P notAlive".
    :return: A Plotly Figure object containing the indicator chart.
    """
    # Churn status for current period based on 'P notAlive' probability
    current_data["churn"] = current_data["P notAlive"].apply(lambda x: 1 if x > 0.5 else 0)
    # Churn status for previous period
    previous_data["churn"] = previous_data["P notAlive"].apply(lambda x: 1 if x > 0.5 else 0)
    ...
    churn_data = current_data.groupby("Valuation Date")["churn"].sum().reset_index()
    # Extracting the month from the 'Valuation Date'
